...
const REGISTER = gql`mutation Register($name: String!, $email: String!, $password: String!) { register(name: $name, email: $email, password: $password) { token user { id name email role } } }`;
...
export default function Register({ onRegister }: Props) {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault(); if (password.length < 6) return setError("Password must be at least 6 characters.");
    setError(""); setLoading(true);
    try { const data = await graphqlClient.request<Response>(REGISTER, { name: name.trim(), email: email.trim(), password }); localStorage.setItem("token", data.register.token); localStorage.setItem("user", JSON.stringify(data.register.user)); setAuthToken(data.register.token); onRegister(data.register.user); }
    catch (err) { setError(err instanceof Error ? err.message : "Unable to create account."); }
    finally { setLoading(false); }
  }
  return <form className="form" onSubmit={submit}>
    <div className="form-heading"><p className="eyebrow">Get started</p><h2>Create account</h2><p>Set up your support workspace in seconds.</p></div>
    ...
    <label>Password<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 6 characters" minLength={6} autoComplete="new-password" required /></label>
